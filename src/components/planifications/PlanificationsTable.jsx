...
import { deletePlanification } from "../../api/axios";
import { useSnackbar } from "notistack";
import { Delete, Plagiarism } from "@mui/icons-material";
import routes from "../../router/routes";

const PlanificatiosTable = ({
  planifications,
  planificationsPages,
  page,
  setPage,
  ...
  const handleDelete = async (data) => {
    const response = await deletePlanification(data);
    if (response.success) {
        ...
        <TableContainer component={Paper} sx={{ mb: 5, pb: 3 }}>
          ...
          <Table sx={{ minWidth: 500, mb: 2 }}>
            ...
            <TableBody sx={{ minHeight: 500 }}>
              {planifications && planifications[0]
                ? planifications.map((row, index) => (
                    <TableRow
                      key={index}
                      ...
                      <TableCell>
                        ...
                        >
                          <Grid item xs={12} md={3}>
                            <Link to={`${routes.planifications_list}/${row._id}`}>
                              <Tooltip
          ...
          <Pagination
            pageNumber={page}
            totalCount={planificationsPages * 15}
...
export default PlanificatiosTable;
